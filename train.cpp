...
{ 
    ...
    //	glRasterPos3f(450,950, 0.0);
    	char c[100]="CGV MINI PROJECT";
    	char c1[100]="TITLE : RUNNING TRAINS";
    	char c2[100];
    	char c3[100]="UNDER THE GUIDENCE OF Your Guide Name";
    	char c4[100]="SUBMITTED BY : BHOOMIKA BR AND MALAVIKA CHENGAPPA K";
    	char c5[100]="Your Name              USN : Your USN";
    	char c6[100]="PRESS 'S' TO START";
	int i;
    	for (i=0; c[i] != '\0'; i++)
    		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, c[i]); // Updates the position
    		glColor3f(0.0,0.0,1.0);
			glRasterPos3f(415,900, 0.0);
		for (i=0; c1[i] != '\0'; i++)
    		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, c1[i]);
    		glColor3f(0.0,1.0,0.0);
			glRasterPos3f(415,820, 0.0);
		for (i=0; c2[i] != '\0'; i++)
    		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, c2[i]);
    		glColor3f(1.0,0.5,1.0);
			glRasterPos3f(325,740, 0.0);
		for (i=0; c3[i] != '\0'; i++)
    		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, c3[i]);
    		glColor3f(1.0,1.0,0.5);
			glRasterPos3f(300,660, 0.0);
		for (i=0; c4[i] != '\0'; i++)
    		glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, c4[i]);
    		glColor3f(0.5,1.0,1.0);
			glRasterPos3f(450,600, 0.0);
		for (i=0; c5[i] != '\0'; i++)
...
void TRAINS2(int x2, int y2,int s,int t)
{
	int r=0;
		...
		glBegin(GL_QUADS);
		glColor3f(0,0.0,1.0);			 //ENGINE
		glVertex2f(x2,y2);				//lengh of engine=60;height of engine=30;
		glColor3f(0,0.0,1.0);
		glVertex2f(x2-60,y2);			
		glColor3f(1.0,0.0,0.0);
